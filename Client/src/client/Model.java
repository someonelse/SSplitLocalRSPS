...
public final class Model extends Animable {
	...
	private static byte[] getModel(int Index) {
		try {
			File Model = new File("C:/SoulSplit1/Models/"+Index+".dat");
	...
	private void method484(int i) {
	    ...
	    Texture.anInt1465 = (anIntArray1639 == null) ? 0 : anIntArray1639[i];

	    int shadingMode = (anIntArray1637 == null) ? 0 : anIntArray1637[i] & 3;

	    if (shadingMode == 0) {
	        Texture.drawAffineTexturedTriangle(
	            anIntArray1666[j], anIntArray1666[k], anIntArray1666[l], // Y coordinates
	            anIntArray1665[j], anIntArray1665[k], anIntArray1665[l], // X coordinates
	            anIntArray1634[i], anIntArray1635[i], anIntArray1636[i], // Texture UVs
	            anIntArray1640[i]  // Texture index — ✅ ADDED parameter
	        );
	        return;
	    }
	    ...
	    if (shadingMode == 2) {
	        // 1) Get the three vertex indices
	         j = anIntArray1631[i];
	         k = anIntArray1632[i];
	         l = anIntArray1633[i];

	        // 2) Extract screen coords
	        int x0 = anIntArray1665[j], y0 = anIntArray1666[j];
	        int x1 = anIntArray1665[k], y1 = anIntArray1666[k];
	        int x2 = anIntArray1665[l], y2 = anIntArray1666[l];

	        // 3) Extract fixed–point UVs (16.16)
	        int u0 = anIntArray1634[i], u1 = anIntArray1635[i], u2 = anIntArray1636[i];
	        int v0 = anIntArray1637[i], v1 = anIntArray1638[i], v2 = anIntArray1639[i];

	        // 4) Convert those to normalized floats
	        float u0f = u0 / 65536f, u1f = u1 / 65536f, u2f = u2 / 65536f;
	        float v0f = v0 / 65536f, v1f = v1 / 65536f, v2f = v2 / 65536f;

	        // 5) Supply a W (depth) value—1.0f if you don’t have per-vertex depth
	        float w0f = 1.0f, w1f = 1.0f, w2f = 1.0f;

	        // 6) Grab the texture pixel array
	        int texIndex = anIntArray1640[i];
	        int[] tex = Texture.anIntArrayArray1483[texIndex];

	        // 7) Finally call the float‐based perspective triangle
	        Texture.drawPerspectiveTriangle(
	            x0, y0, u0f, v0f, w0f,
	            x1, y1, u1f, v1f, w1f,
	            x2, y2, u2f, v2f, w2f,
	            tex
	    ...
	    if (shadingMode == 3) {
	        int triIndex = anIntArray1637[i] >> 2;
	        int v1 = anIntArray1643[triIndex];
	        int v2 = anIntArray1644[triIndex];
	        int v3 = anIntArray1645[triIndex];

	        Texture.drawPerspectiveTriangle(
	            anIntArray1666[j], anIntArray1666[k], anIntArray1666[l],
	            anIntArray1665[j], anIntArray1665[k], anIntArray1665[l],
	            anIntArray1634[i], anIntArray1634[i], anIntArray1634[i],
	            anIntArray1668[v1], anIntArray1668[v2], anIntArray1668[v3],
	            anIntArray1669[v1], anIntArray1669[v2], anIntArray1669[v3],
	            anIntArray1670[v1], anIntArray1670[v2], anIntArray1670[v3],
	            anIntArray1640[i]
	        );
	    }
	}

	// Inside your Model.java, within the Model class:
	private void method485(int faceIndex) {
	    // Screen‐space origin offsets (from Texture)
	    int baseX = Texture.textureInt1;
	    int baseY = Texture.textureInt2;
	
	    // Which three vertices make up this face?
	    int v0 = anIntArray1631[faceIndex];
	    int v1 = anIntArray1632[faceIndex];
	    int v2 = anIntArray1633[faceIndex];
	
	    // Their depths in camera‐space
	    int d0 = anIntArray1670[v0];
	    int d1 = anIntArray1670[v1];
	    int d2 = anIntArray1670[v2];
	
	    // We'll collect up to 4 clipped vertices here:
	    int[] xs = new int[4];
	    int[] ys = new int[4];
	    int[] us = new int[4];
	    int count = 0;
	
	    // Convenience: original screen coords and u‐coords for each vertex
	    int sx0 = anIntArray1665[v0], sy0 = anIntArray1666[v0], u0 = anIntArray1634[faceIndex];
	    int sx1 = anIntArray1665[v1], sy1 = anIntArray1666[v1], u1 = anIntArray1635[faceIndex];
	    int sx2 = anIntArray1665[v2], sy2 = anIntArray1666[v2], u2 = anIntArray1636[faceIndex];
	
	    // Helper to push a vertex
	    final int[] idx = { 0 };

	    Consumer<int[]> push = vert -> {
	        xs[idx[0]] = vert[0];
	        ys[idx[0]] = vert[1];
	        us[idx[0]] = vert[2];
	        idx[0]++;    // mutate the array slot instead of reassigning idx
	    };


	
	    // If vertex 0 is in front of near plane, keep it
	    if (d0 >= 50) {
	        push.accept(new int[]{sx0, sy0, u0});
	    }
	    // Clip edge 0→1
	    if ((d0 < 50) ^ (d1 < 50)) {
	        int t = (50 - d0) * modelIntArray4[Math.abs(d1 - d0)];
	        int cx = baseX + (((sx1 - sx0) * t >> 16) << 9) / 50;
	        int cy = baseY + (((sy1 - sy0) * t >> 16) << 9) / 50;
	        int cu = u0 + ((u1 - u0) * t >> 16);
	        push.accept(new int[]{cx, cy, cu});
	    }
	    if (d1 >= 50) {
	        push.accept(new int[]{sx1, sy1, u1});
	    }
	    // Clip edge 1→2
	    if ((d1 < 50) ^ (d2 < 50)) {
	        int t = (50 - d1) * modelIntArray4[Math.abs(d2 - d1)];
	        int cx = baseX + (((sx2 - sx1) * t >> 16) << 9) / 50;
	        int cy = baseY + (((sy2 - sy1) * t >> 16) << 9) / 50;
	        int cu = u1 + ((u2 - u1) * t >> 16);
	        push.accept(new int[]{cx, cy, cu});
	    }
	    if (d2 >= 50) {
	        push.accept(new int[]{sx2, sy2, u2});
	    }
	    // Clip edge 2→0
	    if ((d2 < 50) ^ (d0 < 50)) {
	        int t = (50 - d2) * modelIntArray4[Math.abs(d0 - d2)];
	        int cx = baseX + (((sx0 - sx2) * t >> 16) << 9) / 50;
	        int cy = baseY + (((sy0 - sy2) * t >> 16) << 9) / 50;
	        int cu = u2 + ((u0 - u2) * t >> 16);
	        push.accept(new int[]{cx, cy, cu});
	    }
	
	    // Now we have count = 3 or 4 vertices in xs[], ys[], us[].
	    // Compute their area to see if front‐facing:
	    int x0c = xs[0], x1c = xs[1], x2c = xs[2];
	    int y0c = ys[0], y1c = ys[1], y2c = ys[2];
	    int area = (x0c - x1c) * (y2c - y1c) - (y0c - y1c) * (x2c - x1c);
	    if (area <= 0) return;        // back‐facing
	
	    // Figure out shading mode for this face
	    int shade = (anIntArray1637 == null ? 0 : anIntArray1637[faceIndex] & 3);
	    int texIdx = anIntArray1640[faceIndex];
	    ...
	    if (count == 3) {
	        ...
	        if (shade == 0) {
	            ...
	            int color = modelIntArray3[anIntArray1634[faceIndex]];
	            Texture.drawFlatTriangle(
	                ys[0], ys[1], ys[2],
	                xs[0], xs[1], xs[2],
	                color
	            ...
	            // Perspective‐correct
	            // reinterpret us[] as u0,u1,u2 and reuse anIntArray1668/1669/1670 for v/w
	            Texture.drawPerspectiveTriangle(
	                ys[0], ys[1], ys[2],
	                xs[0], xs[1], xs[2],
	                us[0], us[1], us[2],
	                anIntArray1668[v0], anIntArray1668[v1], anIntArray1668[v2],
	                anIntArray1669[v0], anIntArray1669[v1], anIntArray1669[v2],
	                anIntArray1670[v0], anIntArray1670[v1], anIntArray1670[v2],
	                texIdx
	            );
	        }
	    } else if (count == 4) {
