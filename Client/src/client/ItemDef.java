...
/* Hardcoded items start here */
	...
	public static Sprite getSprite(int id, int quantity, int outlineColor) {
	    Sprite cached = null;

	    // 1) Try MRU cache when no outline
	    if (outlineColor == 0) {
	        Object maybe = mruNodes1.insertFromCache(id);
	        if (maybe instanceof Sprite) {
	            cached = (Sprite) maybe;
	            if (cached.drawOffsetY != quantity
	             && cached.drawOffsetY != -1) {
	                cached = null;
	            }
	        }
	        if (cached != null)
	    ...
	    // 2) Load ItemDef and handle stackable variants
	    ...
	    if (quantity > 1) {
	        int overrideId = -1;
	        for (int i = 0; i < 10; i++) {
	            if (itemDef.stackAmounts[i] != 0
	             && quantity >= itemDef.stackAmounts[i]) {
	                overrideId = itemDef.stackIDs[i];
	            }
	        }
	        if (overrideId != -1)
	            itemDef = forID(overrideId);
	    }

	    // 3) Render 3D model into a 32×32 sprite
	    Model model = itemDef.method201(1);
	    if (model == null)
	        return null;

	    // Certificate template sprite (drawn on top later)
	    Sprite template = null;
	    if (itemDef.certTemplateID != -1) {
	        template = getSprite(itemDef.certID, 10, -1);
	        if (template == null)
	            return null;
	    }

	    // 4) Prepare our render buffer
	    Sprite render = new Sprite(32, 32);

	    // Backup global drawing state
	    int[] oldTexArr = Texture.anIntArray1472;
	    int oldT1      = Texture.textureInt1;
	    int oldT2      = Texture.textureInt2;
	    int[] oldPix   = DrawingArea.pixels;
	    int oldW       = DrawingArea.width;
	    int oldH       = DrawingArea.height;
	    int oldTopX    = DrawingArea.topX;
	    int oldBotX    = DrawingArea.bottomX;
	    int oldTopY    = DrawingArea.topY;
	    int oldBotY    = DrawingArea.bottomY;

	    // Initialize a 32×32 drawing area on our sprite's pixel buffer
	    Texture.aBoolean1464 = false;
	    DrawingArea.initDrawingArea(32, 32, render.myPixels);
	    DrawingArea.drawPixels(32, 0, 0, 0, 32);
	    Texture.method364();

	    // 5) Project the model
	    double zoomFactor = itemDef.modelZoom;
	    if (outlineColor == -1) zoomFactor *= 1.5;
	    if (outlineColor >  0) zoomFactor *= 1.04;

	    int zoom = (int) zoomFactor;
	    int xr = Texture.anIntArray1470[itemDef.modelRotation1] * zoom >> 16;
	    int yr = Texture.anIntArray1471[itemDef.modelRotation1] * zoom >> 16;
	    ...
	    model.method482(
	        ...
	        xr + model.modelHeight / 2 + itemDef.modelOffset2,
	        yr + itemDef.modelOffset2
	    );

	    // 6) Outline pass (unchanged)
	    for (int y = 31; y >= 0; y--) {
	        for (int x = 31; x >= 0; x--) {
	            int idx = x + y * 32;
	            if (render.myPixels[idx] == 0) {
	                if (x > 0  && render.myPixels[idx - 1] > 1)
	                    render.myPixels[idx] = 1;
	                else if (y > 0 && render.myPixels[idx - 32] > 1)
	                    render.myPixels[idx] = 1;
	                else if (x < 31 && render.myPixels[idx + 1] > 1)
	                    render.myPixels[idx] = 1;
	                else if (y < 31 && render.myPixels[idx + 32] > 1)
	                    render.myPixels[idx] = 1;
	            }
	        }
	    }
	    if (outlineColor > 0) {
	        for (int y = 31; y >= 0; y--) {
	            for (int x = 31; x >= 0; x--) {
	                ...
	                if (render.myPixels[idx] == 0) {
	                    if (x > 0  && render.myPixels[idx - 1] == 1)
	                        render.myPixels[idx] = outlineColor;
	                    else if (y > 0 && render.myPixels[idx - 32] == 1)
	                        render.myPixels[idx] = outlineColor;
	                    else if (x < 31 && render.myPixels[idx + 1] == 1)
	                        render.myPixels[idx] = outlineColor;
	        ...
	        for (int y = 31; y >= 0; y--) {
	            for (int x = 31; x >= 0; x--) {
	                ...
	                if (render.myPixels[idx] == 0
	                 && x > 0 && y > 0
	                 && render.myPixels[idx - 33] > 0) {
	                    render.myPixels[idx] = 0x302020;
	                }
	            }
	        }
	    }

	    // 7) Certificate overlay onto the same 32×32 buffer
	    if (template != null) {
	        int backupW = template.drawOffsetX;
	        int backupH = template.drawOffsetY;
	        template.drawOffsetX = 32;
	        template.drawOffsetY = 32;

	        // <<< key change: pass a third argument to match Sprite.drawSprite( int, int, int )
	        template.drawSprite(0, 0, 0);

	        template.drawOffsetX = backupW;
	        template.drawOffsetY = backupH;
	    }

	    // 8) (Optional) cache/store logic
	    if (outlineColor == 0) {
	        // keep old MRUNodes call or replace with your own Map-based cache
	        // mruNodes1.removeFromCache( … );
	    }

	    // 9) Restore global drawing state
	    DrawingArea.initDrawingArea(oldH, oldW, oldPix);
	    DrawingArea.setDrawingArea(oldBotY, oldTopX, oldBotX, oldTopY);
	    Texture.textureInt1      = oldT1;
	    Texture.textureInt2      = oldT2;
	    Texture.anIntArray1472   = oldTexArr;
	    Texture.aBoolean1464     = true;

	    // 10) Final sprite metadata
	    render.drawOffsetX = itemDef.stackable ? 33 : 32;
	    render.drawOffsetY = quantity;
	    return render;
	}
