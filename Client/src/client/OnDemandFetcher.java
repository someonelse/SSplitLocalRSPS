	...
	implements Runnable
{
    ...
    private void readData()  {
		try {
			...
			{
			    Decompressor[] decs = clientInstance.getDecompressors();
			    int idx = current.dataType + 1;

			    if (decs != null && idx >= 0 && idx < decs.length && decs[0] != null && decs[idx] != null) {
			        decs[idx].method234(abyte0.length, abyte0, current.ID);
			    }
	...
	try
	{
		...
		onDemandCycle++;
		int i = 20;
		if (anInt1332 == 0 && clientInstance.getDecompressors()[0] != null)
		    i = 50;
		try
		{
		    Thread.sleep(i);
