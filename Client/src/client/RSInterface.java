...
public final class RSInterface {
	...
	private static final Map<Long, Sprite> customSpriteCache = new HashMap<>();

	private static Sprite CustomSpriteLoader(int id, String suffix) {
	    // build the same key you were using
	    long key = (TextClass.method585(suffix) << 8) + (long) id;

	    // 1) Try the cache
	    Sprite sprite = customSpriteCache.get(key);
	    if (sprite != null) {
	        return sprite;
	    }

	    // 2) Load from disk
	    String path = "Attack/" + id + suffix;
	    try {
	        sprite = new Sprite(path);
	        customSpriteCache.put(key, sprite);
	        return sprite; // Add this return statement to avoid reaching the second declaration of sprite
	    } catch (Exception e) {
	        // if loading fails, return null
	        return null;
	    }

	    // The following block is redundant and can be removed since the method will return before reaching here
	    /*
	    // 1) try map
	    sprite = customSpriteCache.get(key);
	    if (sprite != null) {
	        return sprite;
	    }
	    ...
	    // 2) load new sprite
	    try {
	        sprite = new Sprite("/Attack/" + id + s); // `s` is undefined, replace with `suffix` if that was intended
	        customSpriteCache.put(key, sprite);
	        return sprite;
	    } catch (Exception e) {
	        // failed to load
	...
	public static void prayerTab(TextDrawingArea[] tda) {
        ...
        addPrayer(18012, 0, 607, 59, 6, "Piety");
...
public static void addText(int i, String s,int k, boolean l, boolean m, int a,TextDrawingArea[] TDA, int j) {
	...
	public static RSInterface addTabInterface(int id) {
	  ...
	  /**
	   * Loads Attack icons (e.g. 150.png, 150a.png) from ./Data/Sprites/Attack/
	   * Caches them in a plain HashMap keyed by (suffix<<8)|id.
	   */
	  private static Sprite imageLoader(int id, String suffix) {
	    long key = (TextClass.method585(suffix) << 8) + (long) id;
	    ...
	    String path = "Attack/" + id + suffix;
	    try {
	      sprite = new Sprite(path);           // uses your new Sprite(String) ctor
	      customSpriteCache.put(key, sprite);
	      return sprite;
	    } catch (Exception e) {
	...
	private Model method206(int i, int j)
	{
		...
		if(i == 3)
			model = client.getMyPlayer().method453();
		if(i == 4)
	...
	private static Sprite method207(int i, StreamLoader streamLoader, String s) {
	    ...
	    Sprite sprite = imageLoader(i, s); // Assuming imageLoader returns a Sprite object
	    if (sprite != null)
	        return sprite;
	    try {
	        // Use an existing constructor or method to create a Sprite from streamLoader
	        // You may need to define a new method or use an alternative approach
	        // Example: sprite = loadSpriteFromStream(streamLoader, s, i);
	        sprite = new Sprite("./Sprites/" + s + " " + i + ".PNG"); // Use a path-based constructor as an example
	    } catch (Exception _ex) {
	        return null;
	    }
	    return sprite;
	...
	private static Sprite LoadSprite(int i, String s) {
	    long l = (TextClass.method585(s) << 8) + (long) i;
	    Sprite sprite = customSpriteCache.get(l); // Declare sprite and attempt to get it from the cache
	    if (sprite != null) {
	        return sprite;
	    }
	    try {
	        sprite = new Sprite("./Sprites/" + s + " " + i + ".PNG");
	        customSpriteCache.put(l, sprite); // Add sprite to cache after loading
	...
	public static void configureLunar(TextDrawingArea[] TDA){
			...
			addLunar3RunesSmallBox(30064, 0, 0, 0, 0, 0, 0, 30012, 30006, 68, "Monsters","Monsters teleport",TDA,6,0,5);
			addLunar3RunesBigBox(30075, 0, 0, 0, 0, 0, 0, 30012,  30006, 69,"Minigames", "Teleport to various Mini games.",TDA, 7,0,5);
			addLunar3RunesSmallBox(30083, 553, 563, 557, 0, 0, 0, 30012, 30006, 70, "Boss's","Loads of differnt boss's",TDA,8,0,5);
			...
			addLunar3RunesSmallBox(30106, 553, 563, 555,  0, 0,0, 30012, 30004, 71,"Pking", "Teleport to various Pking spots.",TDA,11,0,5);
			addLunar3RunesBigBox(30114, 553, 563, 555, 0, 0, 0, 30012, 30004, 72,"Skillz", "various skilling area's.",TDA, 12,0,5);
