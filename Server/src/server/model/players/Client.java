...
public void HighAndLow(){
	...
	public void initialize() {
		synchronized (this) {
			...
			//if (playerName.equalsIgnoreCase("evo")) {
...
setSidebarInterface(6, 12855); // ancient
			...
			sendMessage("Welcome to Soul Split.");
	...
	public void process() {
		...
		double damage4 = 0;
			...
			if(playerIndex > 0) {
				getCombat().applyPlayerClawDamage(playerIndex, 1, previousDamage / 2);
			}
			...
			if(playerIndex > 0) {
				getCombat().applyPlayerClawDamage(playerIndex, 2, previousDamage);
			}
			clawDelay = 0;
			specEffect = 0;
			previousDamage = 0;
		...
		if(inWild() && !isInArd() && !isInFala()) {
			...
			if(Config.SINGLE_AND_MULTI_ZONES) {
				if(inMulti()) {
					getPA().sendFrame126("@gre@Safe Zone", 199);
				} else {
					getPA().sendFrame126("@gre@Safe Zone", 199);
				}
			} else {
