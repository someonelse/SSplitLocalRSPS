	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		switch (packetType) {
			...
			case ATTACK_PLAYER -> {
				...
				if (Server.playerHandler.players[c.playerIndex] == null || c.respawnTimer > 0) break;

				if (c.autocastId > 0) c.autocasting = true;
				if (!c.autocasting && c.spellId > 0) c.spellId = 0;
				...
				if (!usingBow) c.usingBow = false;
				if (!usingOtherRangeWeapons) c.usingRangeWeapon = false;
				...
				if (c.getCombat().correctBowAndArrows() < c.playerEquipment[c.playerArrows]
						&& Config.CORRECT_ARROWS && usingBow && !c.getCombat().usingCrystalBow()
						&& c.playerEquipment[c.playerWeapon] != 9185) {
					c.sendMessage("You can't use " + c.getItems().getItemName(c.playerEquipment[c.playerArrows]).toLowerCase()
							+ "s with a " + c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase() + ".");
					c.stopMovement();
			...
			case MAGE_PLAYER -> {
				...
				if (!c.getCombat().checkReqs()) break;
				...
				for (int r = 0; r < c.REDUCE_SPELLS.length; r++) {
					if (Server.playerHandler.players[c.playerIndex].REDUCE_SPELLS[r] == c.MAGIC_SPELLS[c.spellId][0]) {
						if ((System.currentTimeMillis() - Server.playerHandler.players[c.playerIndex].reduceSpellDelay[r])
								< Server.playerHandler.players[c.playerIndex].REDUCE_SPELL_TIME[r]) {
				...
				if (System.currentTimeMillis() - Server.playerHandler.players[c.playerIndex].teleBlockDelay
						< Server.playerHandler.players[c.playerIndex].teleBlockLength
