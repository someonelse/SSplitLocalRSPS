...
public class PestControl {
    ...
    public void startGame() {
        if (playersInBoat() > 2) {
            ...
            for (int j = 0; j < Server.playerHandler.players.length; j++) {
                if (Server.playerHandler.players[j] != null && Server.playerHandler.players[j].inPcBoat()) {
                    movePlayer(j);
                }
            }
        } else {
    ...
    public void endGame(boolean won) {
        ...
        waitTimer = WAIT_TIMER;
        for (int j = 0; j < Server.playerHandler.players.length; j++) {
            if (Server.playerHandler.players[j] != null && Server.playerHandler.players[j].inPcGame()) {
                Client c = (Client) Server.playerHandler.players[j];
        ...
        for (int j = 0; j < Server.npcHandler.npcs.length; j++) {
            if (Server.npcHandler.npcs[j] != null &&
                Server.npcHandler.npcs[j].npcType >= 3777 && Server.npcHandler.npcs[j].npcType <= 3780) {
    ...
    public boolean allPortalsDead() {
        ...
        for (int j = 0; j < Server.npcHandler.npcs.length; j++) {
            if (Server.npcHandler.npcs[j] != null &&
                Server.npcHandler.npcs[j].npcType >= 3777 && Server.npcHandler.npcs[j].npcType <= 3780 &&
                Server.npcHandler.npcs[j].needRespawn) {
                count++;
            }
        }
        return count >= 4;
